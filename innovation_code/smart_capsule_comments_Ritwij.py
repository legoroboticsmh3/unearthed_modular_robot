...
def read_dht():
	#4=the pin it is conneced to in rasp pi
	humidity_temp_sensor = 4
	# to round up to the nearest tent value
...
# safety state based on temp and rh
def _safety_state(temp, rh):
	...
	if (16.0 <= temp <= 26.0) and (40.0 <= rh <= 58.0):
		return "YELLOW"
	return "RED"
	# the types of boxes/buttons on the screen
...
def make_status_image(temp, rh):
	...
	# status area (simpler and fixed typos)
	status_x = 320
	# heading
	d.text((status_x, 110), "Capsule Status", fill=(255,255,0), font=_font(30))
	# formatted temperature / humidity text (show -- when missing)
	t_txt = "--" if temp is None else f"{temp:.1f} °C"
	h_txt = "--" if rh is None else f"{rh:.0f} % RH"
	d.text((status_x, 160), f"Temperature: {t_txt} (Target 18–24 °C)", fill=(220,220,220), font=_font(22))
	d.text((status_x, 198), f"Humidity: {h_txt} (Target 46–52%)", fill=(220,220,220), font=_font(22))
...
# ------- ORB gallery --------  
#takes pictures from the gallery and uses it to better know what the artifact
...
# -------- Tkinter App -------
#	Hadells everything, keeps everything up to date and help the camera and sensors work together
class App:
...
#------ camera control
	...
	# to know if the thing starts up
